...
def download_data(tickers: List[str], interval: str, start: str, end: Optional[str]) -> pd.DataFrame:
    """
    ...
    """
    try:
        data = yf.download(tickers=tickers, interval=interval, start=start, end=end)['Adj Close']

        if data.empty:
            raise ValueError("No data returned from yfinance. Please check your tickers and date range.")
        return data
    except Exception as e:
        logging.error(f"Error downloading data: {e}")
        raise

def save_to_csv(data: Union[pd.DataFrame, pd.Series], filename: Optional[str], description: str) -> None:
    """
...
def main() -> None:
    ...
    parser = ArgumentParser(description='Download stock data and calculate statistics.')
    parser.add_argument('--tickers', type=str, default='input/tickers.csv', help='CSV file containing the tickers')
    parser.add_argument('--interval', type=str, default='1d', help='The interval for historical data (e.g., 1d, 1wk)')
    parser.add_argument('--start', type=str, default='2020-01-01', help='Start date for historical data (YYYY-MM-DD)')
    parser.add_argument('--end', type=str, help='End date for historical data (YYYY-MM-DD)')
    parser.add_argument('--adj_close', type=str, help='CSV file to save the adjusted close prices')
    parser.add_argument('--returns', type=str, help='CSV file to save the daily returns')
    parser.add_argument('--expected_returns', type=str, help='CSV file to save the expected returns')
    parser.add_argument('--covariance_matrix', type=str, help='CSV file to save the covariance matrix')
    ...
    if not os.path.exists(args.tickers):
        ...
        raise FileNotFoundError(f"The tickers file '{args.tickers}' does not exist.")

    # Read tickers from the CSV file
    try:
        tickers = pd.read_csv(args.tickers).iloc[:, 0].dropna().astype(str).tolist()
    except Exception as e:
        logging.error(f"Error reading tickers from file: {e}")
        raise

    if not tickers:
        raise ValueError("The tickers list is empty. Please check the CSV file.")

    # Download historical adjusted close prices
    adj_close = download_data(tickers, args.interval, args.start, args.end)
    ...
    if args.returns or args.expected_returns or args.covariance_matrix:
        ...
        if args.expected_returns:
            ...
            save_to_csv(expected_returns, args.expected_returns, "expected returns")
        ...
        if args.covariance_matrix:
            ...
            save_to_csv(covariance_matrix, args.covariance_matrix, "covariance matrix")
