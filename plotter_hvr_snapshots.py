...
time_per_solver, hvr_per_solver, hvr_per_snapshot = load_snapshots("hvr_snapshots")
...
for i, solver in enumerate(solvers):
  ...
  y = [mean(hvr_per_solver[solver][j]) for j in range(num_snapshots)]
...
for extension in ("png", "pdf"):
  plt.savefig(bbox_inches='tight', format=extension,
              fname=path("hvr_snapshots", "hvr_mean_snapshots." + extension))
plt.close()

# The same series as a table, written once rather than once per solver.
data = {'Solver': [], 'Time': [], 'Hypervolume Ratio': [],
        'Standard Deviation': []}
for solver in solvers:
  for j in range(num_snapshots):
    data['Solver'].append(solver)
    data['Time'].append(mean(time_per_solver[solver][j]))
    data['Hypervolume Ratio'].append(mean(hvr_per_solver[solver][j]))
    data['Standard Deviation'].append(stdev(hvr_per_solver[solver][j]))
pd.DataFrame(data).to_csv(path("hvr_snapshots", "hvr_data.csv"), index=False)
...
for i, solver in enumerate(solvers):
  x = [mean(time_per_solver[solver][j]) for j in range(num_snapshots)]
  q = [quartiles(hvr_per_solver[solver][j]) for j in range(num_snapshots)]
  plt.fill_between(x, [value[0] for value in q], [value[2] for value in q],
                   color=colors[i], alpha=0.25)
for i, solver in enumerate(solvers):
  x = [mean(time_per_solver[solver][j]) for j in range(num_snapshots)]
  y = [quartiles(hvr_per_solver[solver][j])[1] for j in range(num_snapshots)]
  plt.plot(x, y, label=solver_labels[solver], marker=(i + 3, 2, 0),
...
for extension in ("png", "pdf"):
  plt.savefig(bbox_inches='tight', format=extension,
              fname=path("hvr_snapshots",
                         "hvr_quartiles_snapshots." + extension))
plt.close()

# One raincloud frame per snapshot; run.sh stitches these into hvr.mp4.
for snapshot in range(num_snapshots):
  if not any(hvr_per_snapshot[snapshot]):
    continue
  plt.figure(figsize=(11, 11))
  plt.title("Multi-Objective Portfolio Optimization Problem",
            fontsize="xx-large")
  plt.xlabel(LABEL, fontsize="x-large")
  pt.half_violinplot(data=hvr_per_snapshot[snapshot], palette=colors,
                     orient="h", width=0.6, cut=0.0, inner=None)
  sns.stripplot(data=hvr_per_snapshot[snapshot], palette=colors, orient="h",
                size=2, zorder=0)
  sns.boxplot(data=hvr_per_snapshot[snapshot], orient="h", width=0.20,
  ...
  plt.yticks(ticks=list(range(len(solvers))),
             ...
             fontsize="large")
  plt.savefig(path("hvr_snapshots", "snapshot_" + str(snapshot) + ".png"),
              format="png")
  plt.close()

report_missing("plotter_hvr_snapshots.py")
