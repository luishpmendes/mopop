...
#
# Usage: ./mhaco-tunner.sh <config_id> <instance_id> <seed> <instance> [params...]
# Output: cost time
#
...
# MHACO imposes no multiple-of-four requirement; the factor is kept so the tuned
# range stays 100..500, matching the other pagmo solvers. It does require the
# solution archive to fit inside the population (ker <= population_size,
# maco.cpp:139-142), which the 100 floor on population_size guarantees for every
# ker in the tuned range.
#
# MHACO has no categorical parameters, so unlike NSPSO and MOEA/D-DE there is
# nothing here to map back from a single token to a spaced pagmo string.
TRANSFORMED_PARAMS=()
i=0
...
# Run solver.
#
# --memory is not merely a mirror of run.sh here, the way it is for NSPSO: it is
# what makes the tuned range of --threshold legal at all. mhaco_solver.cpp:31
# builds pagmo::maco with gen = 1, and maco's constructor requires threshold to
# lie in [1, gen] whenever memory is inactive (maco.cpp:76-79) -- that is,
# threshold == 1 exactly. With memory active the constraint relaxes to
# threshold >= 1 (maco.cpp:80-84). Dropping --memory from this invocation would
# abort every configuration whose threshold exceeds 1.
#
