...
export LC_NUMERIC=C
###############################################################################
# MOEA/D-DE Target Runner for iRace
#
# Usage: ./moead-tunner.sh <config_id> <instance_id> <seed> <instance> [params...]
# Output: cost time
#
...
# MOEA/D imposes no multiple-of-four requirement; the factor is kept so the
# tuned range stays 100..500, matching the other pagmo solvers.
TRANSFORMED_PARAMS=()
i=0
while [ $i -lt ${#PARAMS[@]} ]; do
    if [ "${PARAMS[$i]}" = "--population-size-factor" ]; then
        ...
        # iRace emits categorical values unquoted, so pagmo's spaced string
        # "low discrepancy" would word-split into two argv entries.
        # moead-parameters.txt uses a single token instead; map it back to what
        # pagmo accepts. This cannot call fail(): that helper is defined further
        # down, after $ELAPSED exists, so here it would print a single field.
        #
        # "grid" is not a tunable value -- for 4 objectives it only accepts
        # population sizes of exactly C(H+3, 3), which almost no factor in
        # [25, 125] produces. It is rejected here rather than left to throw
        # inside pagmo.
        case "${PARAMS[$((i + 1))]}" in
            low_discrepancy) GENERATION="low discrepancy" ;;
            random) GENERATION="random" ;;
            *) echo "Inf 0"; exit 0 ;;
        esac
        TRANSFORMED_PARAMS+=("--weight-generation" "$GENERATION")
        i=$((i + 2))
    else
        # --decomposition needs no mapping: tchebycheff, weighted and bi are all
        # single tokens, so they survive iRace's unquoted emission intact.
