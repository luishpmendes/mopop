...
fig, axs = plt.subplots(1, ncols=len(metrics), figsize=(
    12.0 * len(metrics), 12.0 * 1), squeeze=False, num=1, clear=True)
fig.suptitle(
    "Multi-Objective Portfolio Optimization Problem", fontsize=42)
for j in range(len(metrics)):
  ...
  sns.stripplot(data=metrics[j], ax=axs[0][j],
                palette=colors, orient="h", size=2, zorder=0)
  sns.boxplot(data=metrics[j], ax=axs[0][j], orient="h", width=0.20, color="black", zorder=10, showcaps=True, boxprops={
...
fig, axs = plt.subplots(nrows=len(metrics), ncols=len(metrics), figsize=(
    8.0 * len(metrics), 8.0 * len(metrics)), squeeze=False, num=1, clear=True)
fig.set_size_inches(8.0 * len(metrics), 8.0 * len(metrics))
fig.suptitle(
    "Multi-Objective Portfolio Optimization Problem", fontsize=36)
for i in range(len(solvers)):
  for j in range(len(metrics)):
    ...
    sns.kdeplot(data=metrics[j][i], ax=axs[j][j], color=colors[i],
                label=solver_labels[solvers[i]], marker=(i + 3, 2, 0), cut=0)
    ...
    for k in range(len(metrics)):
      if j != k:
        ...
        axs[j][k].scatter(x=metrics[k][i], y=metrics[j][i], color=colors[i],
                          label=solver_labels[solvers[i]], marker=(i + 3, 2, 0), alpha=0.60)
...
for i in range(len(solvers)):
  for seed in seeds:
    filename = os.path.join(dirname, "igd_plus/" +
                            solvers[i] + "_" + str(seed) + ".txt")
    if os.path.exists(filename):
      with open(filename) as csv_file:
        data = csv.reader(csv_file, delimiter=",")
...
fig, axs = plt.subplots(nrows=1, ncols=len(metrics), figsize=(
    12.0 * len(metrics), 12.0 * 1), squeeze=False, num=1, clear=True)
...
fig, axs = plt.subplots(nrows=len(metrics), ncols=len(metrics), figsize=(
    5.0 * len(metrics), 5.0 * len(metrics)), squeeze=False, num=1, clear=True)
