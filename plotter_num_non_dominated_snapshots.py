...
for version in versions:
  ...
  for i in range(len(solvers)):
    filename = os.path.join(
        dirname, "num_non_dominated_snapshots/" + solvers[i] + "_" + version + ".txt")
  ...
  filename = os.path.join(
      dirname, "num_non_dominated_snapshots/" + version + ".png")
...
for i in range(len(solvers)):
  for seed in seeds:
    filename = os.path.join(
        dirname, "num_non_dominated_snapshots/" + solvers[i] + "_" + str(seed) + ".txt")
    if os.path.exists(filename):
      with open(filename) as csv_file:
        data = csv.reader(csv_file, delimiter=" ")
        j = 0
...
plt.legend(loc="best", fontsize="large")
filename = os.path.join(
    dirname, "num_non_dominated_snapshots/num_non_dominated_mean_snapshots.png")
plt.savefig(filename, format="png")
plt.close()
...
plt.legend(loc="best", fontsize="large")
filename = os.path.join(
    dirname, "num_non_dominated_snapshots/num_non_dominated_quartiles_snapshots.png")
plt.savefig(filename, format="png")
plt.close()
